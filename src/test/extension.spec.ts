...
test.describe('Extension Tests', () => {
  ...
  test.describe.parallel('Structural Search Tests', () => {
    ...
    test('Exclude files', async () => {
      // Create a search query, exclude files and 
      await webview.getByRole('textbox', { name: strings.searchTextAreaTitle }).click();
      await webview.getByRole('textbox', { name: strings.searchTextAreaTitle }).fill('div');
      await webview.locator('#searchBox').dispatchEvent('keyup');
      ...
      // Exclude files
      await webview.locator('#searchInAll').check();
      await webview.getByRole('textbox', { name: 'files to exclude' }).click();
      await webview.getByRole('textbox', { name: 'files to exclude' }).fill('sample1*,   **/sample2.html');
      ...
      // Verfiy that the files are excluded
      await page.getByRole('button', { name: '' }).click();;
      expect.soft((await page.getByRole('textbox', { name: 'files to exclude' }).inputValue()).includes('**/sample1*,**/sample2.html')).toBeTruthy();
