/*
    ...
    neglected aspects in the package's algorithm and make it compatible with the sidebar search of VS code.
...
export function convertToRegex(searchText : string) : String{
    ...
    try {
        ...
        for(let query of queries){
            ...
            /*
                Be sure that the provided CSS selector is only one or combined of the followings:
           ...
           regex[index] = regex[index].replace("class=","class\\s*=\\s*").replace("id=","id\\s*=\\s*"); 
            ...
            regex[index] = regex[index].replaceAll(`.*[\\s'"]`, `[^=]*[\\s'"]`).
    ...
    // Create the final RegExp by connecting all generated RegExps with the OR(|) symbol
